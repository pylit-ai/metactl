...
- `cargo audit`: passed.
- `cargo publish -p metactl --dry-run --locked`: run for `0.1.14` before publication.
- `cargo publish -p metactld --dry-run --locked`: run after `metactl = 0.1.14` is published to crates.io.
- `cargo search metactl --limit 5`: verify `metactl = "0.1.14"` after publication.
- `cargo search metactld --limit 5`: verify `metactld = "0.1.14"` after publication.
...
1. Publish `metactl`.
2. Wait for `metactl = 0.1.14` to appear in the crates.io index.
