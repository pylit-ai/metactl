...
Use the deterministic benchmark when a release claim depends on `auto` reducing generated context surface while preserving expected skill and slash-command routes:

```bash
make metactl-surface-benchmark
```

The target writes `tmp/starter-surface-benchmark.json` and validates it against `contracts/schemas/metactl/surface_benchmark_artifact.schema.json`. It compares `minimal`, `auto`, and `full` projection modes, records generated byte/token proxies, checks expected pack recall@3, verifies expected Codex slash commands remain emitted, and fails on route false negatives.

This is local projection proof. It is not provider-backed trace proof of live model task success.
