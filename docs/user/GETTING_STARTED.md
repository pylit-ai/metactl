...
# Installed package `metactl v0.1.13` (executable `metactl`)
```
...
# Installed package `metactld v0.1.13` (executable `metactld`)
```
...
#    Compiling metactl v0.1.13 (...)
#    Compiling metactld v0.1.13 (...)
...
For a real repo, use guided setup first. `--plan` is read-only and shows the
raw equivalent commands that automation can run later.
