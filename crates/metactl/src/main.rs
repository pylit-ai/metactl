...
use project_import::{
    ...
    ProjectImportCommand, ProjectImportModeArg, ProjectImportPlanOptions,
};
use setup::cmd_setup;
...
struct SetupArgs {
    ...
    source: Vec<PathBuf>,
    /// Import setup configuration from another metactl-managed project
    #[arg(long = "import-from", value_name = "PROJECT")]
    import_from: Option<String>,
    /// Browse metactl-managed projects interactively before importing setup configuration
    #[arg(long)]
    browse_projects: bool,
    /// Import mode used with --import-from or --browse-projects
    #[arg(long = "import-mode", value_enum, default_value = "auto")]
    import_mode: ProjectImportModeArg,
    /// Comma-separated import fields (default: role,policy,packs,targets,extends-profile,defaults,artifact-policy)
    #[arg(long = "import-fields", value_name = "FIELDS")]
    import_fields: Option<String>,
    /// Include public source records when importing project configuration
    #[arg(long)]
    include_public_sources: bool,
    /// Ignore scope to recommend after setup
    #[arg(long, value_enum, default_value = "both")]
    ignore_scope: IgnoreScopeArg,
    /// Include private source records during import, or private source ignore patterns in normal setup guidance
    #[arg(long)]
    include_private_sources: bool,
...
enum ProjectCommand {
    ...
    /// Import configuration from another metactl-managed project
    #[command(subcommand)]
...
fn run(cli: &Cli) -> std::result::Result<CommandOutput, CliError> {
    ...
    match &cli.command {
        ...
        Commands::Hook(args) => cli_hook::cmd_hook(cli, args),
        Commands::Source(args) => cli_source::cmd_source(cli, args),
        Commands::Profile(args) => cli_profile::cmd_profile(cli, args),
...
fn mutating_operation_label(cli: &Cli) -> Option<&'static str> {
    match &cli.command {
        ...
        Commands::Project(args) => match &args.command {
            ...
            ProjectCommand::Import(import_args) => match import_args {
...
fn cmd_project(cli: &Cli, args: &ProjectArgs) -> std::result::Result<CommandOutput, CliError> {
    match &args.command {
        ...
        ProjectCommand::Import(import_args) => cmd_project_import(cli, import_args),
    }
}
...
fn ensure_ignore_recovery_dir(project_root: &Path) -> std::result::Result<PathBuf, CliError> {
...
fn recovery_copies_ignored_by_git(project_root: &Path, written: &[(usize, PathBuf)]) -> Result<()> {
    if written.is_empty() {
        return Ok(());
    }
...
use cli_pack::{
    ...
    CodexSkillEntry, SkillFileEntry, SkillFrontmatter,
};
use cli_plugin::cmd_plugin;
use cli_skills::cmd_skills;
use cli_source::{
