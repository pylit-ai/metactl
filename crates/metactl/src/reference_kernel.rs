...
impl ReferenceKernel {
    ...
    /// Inspect real destinations without writing project files. Generated bytes
    /// must already exist under the separately owned staging root.
    pub fn preview_compiled_outputs(
        &self,
        project_root: impl AsRef<Path>,
        staging_root: impl AsRef<Path>,
        manifest: &CompileManifest,
        apply_mode: &ApplyMode,
    ) -> Result<(ApplyReviewPlan, Vec<crate::types::ApplyConflict>)> {
        match &self.backend {
            KernelBackend::Libraries(_) => crate::materializer::preview_manifest(
                project_root.as_ref(),
                staging_root.as_ref(),
                manifest,
                apply_mode,
